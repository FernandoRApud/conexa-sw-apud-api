...
export const getAllPeoples = async (search?: string) => {
  const people = search ? await clientAxios.get(`${ROUTES.PEOPLE}?search=${search}`) : await clientAxios.get(`${ROUTES.PEOPLE}`);
  return people.data;
};
